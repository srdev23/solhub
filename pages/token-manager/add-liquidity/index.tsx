...
import { getRayPoolKeyInfo, addLiquidity } from "@/lib/txHandler";
...
export default function Home() {
  ...
  const getPoolInfo = async () => {
    ...
    try {
      const res = await getRayPoolKeyInfo(new PublicKey(lpAddress), "mainnet");
      if (res) {
        const base = await fetchTokenMetadata(res.baseMint.toBase58());
        console.log("base=>>>", base);

        setBaseToken(base);
        const quote = await fetchTokenMetadata(res.quoteMint.toBase58());
        setQuoteToken(quote);
      }
    } catch (error: any) {
  ...
  const faqData = [
    {
      ...
      description: "The Liquidity Address, also known as the AMM ID, is the unique identifier of the Automated Market Maker (AMM) pool where you want to add liquidity. Entering this address correctly ensures your tokens are added to the right pool."
  },
  {
      label: 'How do I determine the Base and Quote Token amounts?',
      description: "The Base Token and Quote Token amounts represent the quantities of each token you want to add to the liquidity pool. Ensure that you provide equal values of both tokens to maintain a balanced pool."
  },

  {
      label: 'Is it safe to add liquidity?',
      description: "Yes, adding liquidity on our platform is secure. Your transactions are protected by on-chain smart contracts, ensuring that your assets are safely managed throughout the process."
  },
  ]
  ...
  return (
    <>
      <Head>
        <title>Solana Liquidity Pool Adder | Easy & Cheap | SolHub</title>
        <meta name="description" content="Easily add liquidity to your Solana tokens with SolHub's fast and secure liquidity pool adder. Simplify token management with our user-friendly platform designed for seamless Solana operations." />
      </Head>
      <main
...
<p className="text-base1">
    Easily add liquidity to your Solana liquidity pool using our Solana Liquidity Adder. Begin by entering the Liquidity Address (AMM ID) to specify the pool where you want to contribute your tokens.
</p>
<p className="text-base1">
    Next, input the amounts for both the base token and the quote token that you wish to add. These token amounts will directly enhance the liquidity available for trading within the pool, facilitating smoother transactions.
</p>
<p className="text-base1">
    Once you've entered the necessary details, click "Add Liquidity" to contribute your tokens securely. Our platform ensures that your assets are protected throughout the process, making it simple to support the Solana ecosystem.
</p>
