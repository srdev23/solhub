...
import { fetchTokenMetadata, getTokenList, solanaConnection } from "@/lib/utils";
...
import { getRaydiumPoolkeys, raydiumBumpTxn } from "@/base/raydium";
import { moonshotBumpTxn } from "@/base/moonshot";
import { payFeeTxn } from "@/base/feeTxn";
  ...
  const runBumpbot = async () => {
    try {
      ...
      if (validationFlag) {
        ...
        const payerKeypairs = privateKeys.map((pk) =>
        ...
        if(selectedDex === "raydium") {
          bumpData = await getRaydiumPoolkeys(mint);
        }
        toast.success("Started the bot...");
        const feePaid = await payFeeTxn(payerKeypairs[0], "bump");
        if(!feePaid)
          throw new Error("Fee not paid, please check your wallet SOL balance");
        ...
        const executeBotLogic = async () => {
          try{
            ...
            if(selectedDex === "pumpfun"){
              bumpData = await getPumpData(mint);
            }

            const vTxns = await Promise.all(
              payerKeypairs.map(async(payer, index) =>
                {
                  ...
                  const bumpParams = {
                    ...
                    tip: index === 0 ? tipAmount : undefined,
                  }
                  const vTxn = selectedDex === "pumpfun"? await pumpfunBumpTxn(bumpParams)
                    : selectedDex === "raydium"? await raydiumBumpTxn(bumpParams)
                    : await moonshotBumpTxn(bumpParams);
                  if(!vTxn)
                    throw new Error("Failed to create bump transaction");
                  vTxn.sign([payer]);
                  return vTxn;
                }
              )
            ...
            // if (isSucceed) {
              ...
              // console.log(`Bump ${amount} ${ca} success!`, `BundleId: ${bundleId}`);
            // }
          }catch (e: any){
            setBotIsOn(false);
            clearInterval(intervalId);
        ...
        <div className=" w-full h-full flex gap-6">
          ...
          <div className=" w-full max-esm:px-3 p-5 gradient-1 backdrop-blur-lg border  custom-scrollbar h-[84vh] overflow-y-auto    rounded-xl justify-start flex items-center flex-col">
            <span className="text-center w-full text-[25px] flex justify-center font-bold">
              Bump Bot ( Pump.fun / Raydium / Moonshot )
            </span>
                  ...
                  endContent={
                    <div className="flex gap-2 text-[13px]">
                      <span
                        ...
                        onClick={() => setMinTxnAmount(0.000001)}
                      >
                        0.000001
                      </span>
                    </div>
                ...
                <Input
                  ...
                  endContent={
                    <div className="flex gap-2 text-[13px]">
                      <span
                        className="p-1 px-3 rounded-2xl border-[2px] cursor-pointer hover:bg-[#27272a]"
                        onClick={() => setMaxTxnAmount(0.00001)}
                      >
                        0.00001
                      </span>
                    </div>
              ...
              <div className="col-span-12 rounded-xl pt-1">
                <div className="flex flex-col">
                  <div className="text-[14px] sm:text-[11px] md:text-[15px] font-medium mb-1 ml-1">
                    Wallet Private Keys ({privateKeys.length}/5):
                  </div>
                      ...
                      <div key={index} className="flex gap-2 items-center">
                        ...
                        {privateKeys.length > 1 && (
                          <Button
                            ...
                            onClick={() => {
                              setPrivateKeys(
                                privateKeys.filter((_, i) => i !== index)
                              );
                            }}
                          >
                            Remove
              ...
              {botIsOn && (
                ...
                <Button
                  ...
                  onClick={() => {
                    if(botIsOn)
                      toast.warn("Stopped the bot...");
                    botIsOn ? setBotIsOn(false): runBumpbot();
                  }}
                >
