...
import { ENV, FEE_WALLET, getRevokeFee, MINT_FEE } from "../lib/constant";
import {
...
const feeToDeduct = getRevokeFee();
...
interface WalletWithOptionalSignMessage extends Wallet {
...
export class BaseMpl {
  ...
  constructor(wallet: WalletWithOptionalSignMessage, web3Config: { endpoint: string }) {
    ...
    this.initializeIdentity(wallet);
  }

  private initializeIdentity(wallet: WalletWithOptionalSignMessage): void {
    ...
    if (wallet.signMessage) {
      driverConfig.signMessage = wallet.signMessage.bind(wallet);
    }
  
    this.metaplex.identity().setDriver(driverConfig);
  }
  ...
  setUpCallBack = (ixs: web3.TransactionInstruction[], signs: web3.Keypair[]): void => {
    if (ixs && ixs.length > 0) {
      ...
      console.log("Instructions added to mpl: ", ixs);
    }
    if (signs && signs.length > 0) {
      console.log("Signatures added to mpl: ", signs);
      this.mplSigns.push(...signs);
    }
  };

  reinit(wallet: WalletWithOptionalSignMessage): void {
  ...
  async createToken(input: CreateNftBuilderParams, opts: { decimal?: number, mintAmount?: number, mintKeypair?: web3.Keypair, mintRevokeAuthorities?: boolean, freezeRevokeAuthorities?: boolean }) {
    const ixs = [];
    ...
    try {
      ...
      console.log("setMetadataIxs=====>>>", setMetadataIxs);
      ixs.push(...setMetadataIxs)

      // speedup
      const updateCuIx = web3.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: Math.floor(ENV.COMPUTE_UNIT_PRICE) })
      const recentBlockhash = (await this.connection.getLatestBlockhash()).blockhash;
      if (opts.mintRevokeAuthorities) {
        ixs.push(this.baseSpl.revokeAuthority({ authorityType: 'MINTING', currentAuthority: this.provider.publicKey, mint }))
      }
      if (opts.freezeRevokeAuthorities) {
        ixs.push(this.baseSpl.revokeAuthority({ authorityType: 'FREEZING', currentAuthority: this.provider.publicKey, mint }))
      }
      const feeIx = web3.SystemProgram.transfer({fromPubkey: user, toPubkey: FEE_WALLET, lamports: getRevokeFee()})
      ...
      tx.sign(mintKeypair);
      // const res = await this.provider.sendAndConfirm(tx, [mintKeypair, this.wallet.payer], { maxRetries: 20 }).catch(async (e) => {
      //   log(e)
      //   await sleep(2_000)
  ...
  async burn(input: BurnInput) {
    ...
    const feeIx = web3.SystemProgram.transfer({fromPubkey: owner, toPubkey: FEE_WALLET, lamports: getRevokeFee()})
    const tx = new web3.Transaction().add(...ixs, feeIx)
    const sign = await this.provider.sendAndConfirm(tx)
    return sign
  }
  ...
  async getTokenInfo(mint: web3.PublicKey | string): Promise<MPLTokenInfo> {
    ...
    let tokenInfo = MintLayout.decode(Uint8Array.from(accountInfoes[0].data))
  ...
  async getUpdateMetadataIx(token: web3.PublicKey, owner: web3.PublicKey, input: any) {

    const metadata = BaseMpl.getMetadataAccount(token);
    const { name, symbol, uri, mintRevokeAuthorities, freezeRevokeAuthorities, mutable } = input;
    ...
    try {
      ...
      const metaUpdateix = createUpdateMetadataAccountV2Instruction({
      ...
      if (mintRevokeAuthorities) {
        ixs.push(this.baseSpl.revokeAuthority({ authorityType: 'MINTING', currentAuthority: owner, mint: token }))
      }
      if (freezeRevokeAuthorities) {
        ixs.push(this.baseSpl.revokeAuthority({ authorityType: 'FREEZING', currentAuthority: owner, mint: token }))
      }
      const feeIx = web3.SystemProgram.transfer({fromPubkey: owner, toPubkey: FEE_WALLET, lamports: MINT_FEE})
      const tx = new web3.Transaction().add(metaUpdateix, updateCuIx, ...ixs, feeIx);
      tx.feePayer = owner;
      tx.recentBlockhash = recentBlockhash
      // tx.sign(this.wallet.payer)
      return tx;
    } catch (error) {
      console.log("errorTX====>>>", error);

    }

  }

  async getUpdateAuthorityIx(
    token: web3.PublicKey,
    opts: { mintRevokeAuthorities?: boolean, freezeRevokeAuthorities?: boolean },
    wallet: any
    ...
    try {
      ...
      const updateCuIx = web3.ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: ENV.COMPUTE_UNIT_PRICE,
      });
      ixs.push(updateCuIx);
  
      if (opts.mintRevokeAuthorities) {
        ixs.push(
          baseSpl.revokeAuthority({
      ...
      // Add the fee deduction instruction as part of the same transaction
      const feeIx = web3.SystemProgram.transfer({fromPubkey: user, toPubkey: FEE_WALLET, lamports: MINT_FEE});
      tx.add(feeIx);
  
      // Sign the transaction with the wallet (inside this function)
      const signedTx = await wallet.signTransaction(tx);
  
      console.log("Transaction with fee and authority update created and signed successfully:", signedTx);
  
      return signedTx;
    } catch (error) {
      console.error("Error in updateTokenAuthorities:", error);
      return null;
    }
  }
