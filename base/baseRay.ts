...
export class BaseRay {
  ...
  constructor(input: BaseRayInput) {
    ...
    this.pools = new Map();
    if (input.rpcEndpointUrl == "https://api.devnet.solana.com" || input.rpcEndpointUrl == DEV_RPC) {
  ...
  async getMarketInfo(marketId: web3.PublicKey) {
    const marketAccountInfo = await this.connection.getAccountInfo(marketId, 'confirmed').catch((error) => null)
  ...
  async removeLiquidity(input: RemoveLiquidityInput): Promise<Result<{ ixs: web3.TransactionInstruction[] }, string>> {
    ...
    const totalLp = Number(AccountLayout.decode(Uint8Array.from(lpTokenAccountInfo.data)).amount.toString())
    ...
    if (!totalLpD) return { Err: "not have zero lp tokens" }
  ...
  async buyFromPool(input: BuyFromPoolInput): Promise<{ ixs: web3.TransactionInstruction[], signers: web3.Signer[] }> {
    ...
    console.log('buyFromPool amount raw', amountIn.raw)
  ...
  async createPool(input: CreatePoolInput, user: web3.PublicKey) {
    ...
    const baseMintState = MintLayout.decode(Uint8Array.from(baseMintAccountInfo.data));
    const quoteMintState = MintLayout.decode(Uint8Array.from(quoteMintAccountInfo.data));
    ...
    let startTime = new BN(Math.trunc(Date.now() / 1000) - 4)
    if (input.launchTime) startTime = new BN(Math.trunc(input.launchTime));
  ...
  async createMarket(input: CreateMarketInput, user: web3.PublicKey)
    ...
    try {
      ...
      const { baseMint, quoteMint, eventLength, requestLength, orderBookLength } = input;
      ...
      try {
        baseMintDecimals = MintLayout.decode(Uint8Array.from(baseMintAccountInfo.data)).decimals
        quoteMintDecimals = MintLayout.decode(Uint8Array.from(quoteMintAccountInfo.data)).decimals
      } catch (error) {
      ...
      const {
        ...
        eventQueueLength: eventLength,
        requestQueueLength: requestLength,
  ...
  async computeBuyAmount(input: ComputeBuyAmountInput, etc?: { extraBaseResever?: number, extraQuoteReserve?: number, extraLpSupply?: number }) {
    ...
    const lpSupply = new BN(toBufferBE(MintLayout.decode(Uint8Array.from(lpAccountInfo.data)).supply, 8)).addn(etc?.extraLpSupply ?? 0)
    const baseReserve = new BN(toBufferBE(AccountLayout.decode(Uint8Array.from(baseVAccountInfo.data)).amount, 8)).addn(etc?.extraBaseResever ?? 0)
    const quoteReserve = new BN(toBufferBE(AccountLayout.decode(Uint8Array.from(quoteVAccountInfo.data)).amount, 8)).addn(etc?.extraQuoteReserve ?? 0)
  ...
  async computeAnotherAmount({ amount, fixedSide, poolKeys, isRawAmount, slippage }: ComputeAnotherAmountInput) {
    ...
    slippage = slippage ?? new Percent(10, 100)
    ...
    const lpSupply = new BN(Number(MintLayout.decode(Uint8Array.from(lpMintAInfo.data)).supply.toString()))
    const baseReserve = new BN(Number(AccountLayout.decode(Uint8Array.from(baseVAccountInfo.data)).amount.toString()))
    const quoteReserve = new BN(Number(AccountLayout.decode(Uint8Array.from(quoteVAccountInfo.data)).amount.toString()))
